...
//tests the arguement for precedence and returns integer value based on importa$
int pemdas(char op)
{
  if (op == '~' || op == '^')
    return 4;
...
string convert(const string& infix)
{
  ...
  for (size_t i = 0; i < infix.length(); ++i)//loop to go through all infix cha$
  {
    if (isalpha(infix[i])){//checks if character is alphabetic

      if (postfix.length() != 0)//checks if the end of the postfix has been rea$
        postfix += ' ';

      postfix += infix[i];//if character is alphabetic it is added to the postf$
    }
       ...
       for (size_t q = i; q <= infix.length(); ++q){//loop for if numbers have $
       ...
       thePemdas = pemdas(infix[i]);//assigns precedence of infix[i] to an inte$
