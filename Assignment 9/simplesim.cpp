...
bool simplesim::load_program()
{
        ...
        {
           if (instruction > -9999 && instruction < 9999)//accept instruction w$
           {
                if (totalInstructions >= 100)//decision statement for too many $
                {
                ...
                {
                    memory[totalInstructions] = instruction;//assigning valid i$
                    instruction++;
                    totalInstructions++;
                }
           }
...
void simplesim::execute_program()
{
   ...
   while (!done)
   {
    ...
    {
        instruction_register = memory[instruction_counter];//if within range, i$

        operation_code = instruction_register / 100;//operation set to rounded $
    ...
    if (operation_code == HALT)//the final operation that is read to signify th$
    {
    ...
    switch (operation_code)
    {
        case READ:
            ...
            {
                memory[operand] = data;//value in data gets assigned a memory l$

                cout << left << "READ: " << showpos << setfill('0') << internal$
                     << noshowpos << setfill(' ') << "\n";
                break;
            }
   ...
   //instruction counter will increment without special circumstances if operation $
   if (operation_code != BRANCH && operation_code != BRANCHZERO && operation_code !$
        instruction_counter++;
   }
}
...
void simplesim::dump() const
{
        ...
        cout << left << setw(24) << "accumulator:" << showpos << setfill('0') << in$
             << setw(5) << accumulator << noshowpos << setfill(' ') << "\n";

        cout << left << setw(24) <<  "instruction_counter:" << setfill('0') << righ$
             << instruction_counter << setfill(' ') << "\n";

        cout << left << setw(24) << "instruction_register:" << showpos << setfill('$
             << setw(5) << instruction_register << noshowpos << setfill(' ') << "\n$

        cout << left << setw(24) << "operation_code:" << setfill('0') << right << s$
             << operation_code << setfill(' ') << "\n";
        cout << left << setw(24) << "operand:" << setfill('0') << right << setw(2) $
             << setfill(' ') << "\n" << "\n";
        ...
        while (num < 100)//prints all y-coordinates
        {
           if (num % 10 == 0)
           {
                ...
                cout << setw(2) << y_coor << " " << internal << showpos << setfill($
